...
def load_importance_loss(scores_wo_noise, topk_logits, num_global_experts, gate_noise):
    def load_loss(scores_wo_noise, topk_logits, num_global_experts, gate_noise):
        ...
        # Optionally check for NaNs in scores_wo_noise
        if torch.isnan(scores_wo_noise).any():
            raise ValueError("NaN detected in scores_wo_noise before computing diff.")
        diff = scores_wo_noise.float() - threshold.float()
        if torch.isnan(diff).any():
            raise ValueError("NaN detected in diff after subtraction.")
